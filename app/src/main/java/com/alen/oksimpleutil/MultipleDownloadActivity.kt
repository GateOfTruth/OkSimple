...
class MultipleDownloadActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
            ...
            override fun itemClick(position: Int, view: View) {
                when (view.id) {
                    R.id.btn_item_start_download -> {
                        OkSimple.downloadFile(beanList[position].url, beanList[position].fileName, path)
                            .execute(callBack)
                    }
        ...
        (recycler_multiple_download.itemAnimator as SimpleItemAnimator).supportsChangeAnimations=false
    }
